function [xdot,U,M] = remus100_updated_OC(x,ui,Vc,betaVc,w_c)
...
%   2025-08-22  Changed to a more advanced ocean current model with the
%               correct implementation of the ocean current time derivative 
%               (S. Zieglmeier)
%
% References: 
...
%  Sebastian Zieglmeier, et.al., "Data-Enabled Predictive Control and 
...
if length(x) == 12 % Euler angles
    ...
    theta = x(11);
    psi = x(12);
else % Convert unit quaternion to euler angles
    [phi,theta,psi] = q2euler(x(10:13));
end
...
alphaVc = 0;
nu_c_n = Vc * [cos(alphaVc)*cos(betaVc) sin(betaVc) sin(alphaVc)*cos(betaVc)]' + [0 0 w_c]';
nu_c = [(Rzyx(phi, theta, psi))' * nu_c_n; 0; 0; 0]; % Ocean current velocities resulting from the horizontal current

Dnu_c = [-Smtrx(nu(4:6)) * nu_c(1:3); 0; 0; 0];  % Time derivative of nu_c


% Relative velocities/speed, angle of attack and vehicle speed
nu_r = nu - nu_c;                                 % Relative velocity
alpha = atan2( nu_r(3), nu_r(1) );                % Relative Angle of attack (rad)
